...
?>
<?php 
//let's see if we're editing
$edit="0";
$recall="0";
if( isset($_GET['vacancy_id'])&&is_numeric(intval($_GET['vacancy_id'])) ){
	$edit="1";
	$vacancy_id = $_GET['vacancy_id'];
	
	if( $_GET['task']=="recall" ){
		$recall="1";
		$query="select * from $tbl_vacancy where id=".$vacancy_id." and employer_id=".$_GET['employer_id']." and papers='0'";
...
if( isset($_POST['isco_search']) ){
	...
	$isco_1 = ($recall=="0")?$_POST['isco_1']:$row_vacancy['isco_1'];//edit here
	$isco_2 = ($recall=="0")?$_POST['isco_2']:$row_vacancy['isco_2'];
	$isco_3 = ($recall=="0")?$_POST['isco_3']:$row_vacancy['isco_3'];

}

//if we chose among the first result set
...
if (isset($_POST['savevacancy'])){
	...
	//		exit;
		...
		if(insquery($query)){
			include('../access/employer_profile_edited.php');
			exit;
		}else{
...
<p><strong>The employer must complete one form for each request related to the same profile.</strong></p>
Upon compilation - while the entered data is being processed - the employer <em><strong>must</strong></em> print, sign and send each request to:
...
(based on <a href="http://www.ilo.org/public/english/bureau/stat/isco/isco08/" target="_blank">ISCO Standards</a> / <a href="http://ec.europa.eu/eures/home.jsp?lang=en" target="_blank">EURES</a>)*<br />
Use <em><strong>all three drop-down lists</strong></em>, if available, to choose the profile closer to the one you actually need</div></td>
   </tr>
   <tr valign="top">
